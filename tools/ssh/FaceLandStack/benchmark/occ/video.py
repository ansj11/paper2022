...
class TestUnit(TestUnitBase):
    ...
    def gen_kv_dict_all(self, data_root, video_suffix, det_suffix, times, n_video):
        if self._kv_dict_all is None:
            video_suffix = ['.{}'.format(v) for v in video_suffix.split(',')] # add dot
            det_suffix = '.{}'.format(det_suffix) # add dot
    ...
    def gen_inf_outputs_data(self, devices, thr, net, mean_face_file):
        assert self._kv_dict_all, 'Please generate kv_dict_all first'
        if os.path.exists(thr):
            thr0 = []
            thr = list(misc.load_pickle(thr).values())[0]
            for i in range(81):
                s = (list(filter(lambda x: x[2] >= 0.7, thr[i])) or [thr[i][-1]])[0]
                thr0.append(s[-1])
            thr0 = np.float32(thr0)
        else:
            thr0 = np.ones((1, 81)) * float(thr)
        ...
        for key, v in tqdm.tqdm(self._kv_dict_all.items()):
            frames, dets, fps = v
            ...
            occ_pred_sig = output_data[net.outputs[0].name][0]
            occ_pred = (occ_pred_sig >= thr0).astype(np.float32)
            s_pred_val = output_data[net.outputs[1].name][0].reshape(-1, 2) * np.float32([image_width, image_height])
            s_pred_val_raw = np.concatenate([s_pred_val, np.ones((len(s_pred_val), 1))], axis=1).dot(mat_inv.T)[:, :2]
            frames_out = []
            img_copy = draw(img_raw, s_pred_val_raw, s_pred_val_raw[occ_pred==1], rects=[left, top, right, bottom], wait=False, show=False, point_size=2, font_scale=2)
            ...
            for frame in frames[1:]:
                ...
                s_pred_val = output_data[net.outputs[1].name][0].reshape(-1, 2) * np.float32([image_width, image_height])
                s_pred_val_raw = np.concatenate([s_pred_val, np.ones((len(s_pred_val), 1))], axis=1).dot(mat_inv.T)[:, :2]
                img_copy = draw(img_raw, s_pred_val_raw, s_pred_val_raw[occ_pred==1], wait=False, show=False, point_size=2, font_scale=2)
                frames_out.append(img_copy)

            self._inf_outputs_data.setdefault(key, [frames_out, fps])
...
def main(model, devices, args_input, caches):
    ...
    parser.add_argument('--data_root', default='/unsullied/sharefs/_research_facelm/Isilon-datashare/proj/occ/video')
    parser.add_argument('--thr', default='0.5', type=str)
    parser.add_argument('--out_root', default='/tmp/occ.video')
