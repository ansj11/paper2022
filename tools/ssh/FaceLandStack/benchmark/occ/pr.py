...
class TestUnit(TestUnitBase):
    def __init__(self, caches):
        name = "occ.pr Tester ({}-{})".format(time.strftime("%x"), time.strftime("%X"))
    ...
    def compute_metric(self, lm_name, out_root, pr_res_out_file):
        ...
        for key in self._kv_dict_all.keys():
            ...
            for i in range(81):
                ...
                precision, recall, thresholds, auc = misc.compute_precision_recall(probs, gt)
                ps = misc.summary_precision_recall(precision, recall, thresholds, N=10) #(name, precision, recall, threshold)
                pr_res_dict.setdefault(key, []).append(ps)
                comp_name = '{}-{}-{}'.format(key, self.point_to_component[i], i)
                res_dict0 = OrderedDict(
                    [(v[0], '{:.2f}-{:4f}'.format(v[2], v[3])) for v in ps]
                )
                res_dict0['auc'] = auc
                ...
                if out_root != 'None':
                    for idx in range(len(ps)):
                        if ps[idx][-1] != 0.0:
                            break
                    if idx >= len(ps):
                        thr, p0, recall0 = 0.5, 'p0', 0.0
                    else:
                        thr, p0, recall0 = ps[idx][-1], ps[idx][0], ps[idx][2]

                    save_root = os.path.join(out_root, key, '{}-{}-{}-{}-{:.6g}'.format(self.point_to_component[i], i, p0, recall0, thr))
                    misc.ensure_dir(save_root, erase=True)

                    pred = occ_pred_sig[:, i] >= thr
                    ...
                    quick_draw(recall, precision, title, 'recall', 'precision', fig_save_file)
            ...
            if out_root != 'None':
                ...
                title='auc:{:.4g}, p100:{:.4g}, p99:{:.4g}, p98:{:.4g}, p97:{:.4g}'.format(auc, ps[0][2], ps[1][2], ps[2][2], ps[3][2])
...
def main(model, devices, args_input, caches):
    ...
    parser.add_argument('--occ-out-root', default='/tmp/occ.accuracy')
