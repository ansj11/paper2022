...
def encode(eyeocc):
    assert eyeocc in ['glasses', 'sunglasses', 'no_occ', 'undefined', 'others']
    if eyeocc == 'undefined':
        return [0.0, 0.0, 0.0, 0.0]
    if eyeocc == 'sunglasses':
...
class EyeOccImage(object):
    """eye occlusion"""
    def __init__(self, nori_file=None, keys=None):
        self.nori_file = nori_file or '/unsullied/sharefs/_research_facelm/Isilon-datashare/proj/eyeocc/landmark_eyeocc_val_171127.info'
        self.keys = keys
    ...
    def load(self, mean_face_file=None, image_shape=(112, 112),
        ...
        for key in self.keys:
            ...
            img_raw_batch, lm_raw_batch, eye_occ_batch,  nori_id_batch = [], [], [], []
            for nori_id, value in tqdm.tqdm(list(nori_kv_dict.items())[:n_samples0]):
                ...
                eye_occ_batch.append(encode(value['left_eye_occ']) + encode(value['right_eye_occ']))
                nori_id_batch.append(nori_id)
            kv_dict = dict(
                img = img_raw_batch,
                label = lm_raw_batch,
                eyeocc = eye_occ_batch,
        ...
        if align:
            ...
            for key, kv_dict in kv_dict_all.items():
                ...
                kv_dict_align = dict(
                    ...
                    eyeocc = np.float32(kv_dict['eyeocc']),
...
if __name__ == '__main__':
    ...
    for ds_name, kv_dict in kv_dict_all.items():
        img = kv_dict['img']
        label = kv_dict['label']
        eyeocc = kv_dict['eyeocc']
        for img0, label0, eyeocc0 in zip(img, label, eyeocc):
            print(img0.shape)
            print(eyeocc0)
