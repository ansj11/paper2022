...
class tdfawImage(object):
    def __init__(self, nori_file, keys, name='pt3d'):
        ...
        if align:
            ...
            for key, kv_dict in kv_dict_all.items():
                ...
                for img, lm in zip(img_raw, lm_raw):
                    img_out, lm_out, norm_out, mat_out = aug(img, lm, norm_type=norm_type, mat=True)
                    #img_out, lm_out, param_dict_out = aug(img, lm, lm.copy(), align_func=align_func)
                ...
                lm_new = np.array(lm_new).astype('float32').reshape(len(lm_new), -1)
