...
class TestUnit(TestUnitBase):
    ...
    def gen_inf_outputs_data(self, net, devices):
        ...
        warp_imgs = misc.find_opr_by_name(net.outputs, ['left_eye__patch:out', 'right_eye__patch:out'])
    ...
    def compute_metric(self, lm_name, thr, badcase, out_root='/tmp/eyeoc.thr'):
        ...
        for key in self._kv_dict_all.keys():
            ...
            left_eye_patch = (self._inf_outputs_data[key]['left_eye__patch:out'] * 128 + 128).astype(np.uint8).transpose(0, 2, 3,1)
            right_eye_patch = (self._inf_outputs_data[key]['right_eye__patch:out'] * 128 + 128).astype(np.uint8).transpose(0, 2, 3, 1)

            fp = (1-pred) * gt # predict open actually close
            fpr = fp.sum() / gt.sum()
            tp = (1-pred) * (1-gt)
            tpr = tp.sum() / (1-gt).sum()
            fn = (pred * (1-gt)) # predict close actually open
            ...
            if badcase:
                ...
                fp_root = os.path.join(ds_save_root, 'thr{:.3f}-predict_open_actually_close'.format(thr))
                fn_root = os.path.join(ds_save_root, 'thr{:.3f}-predict_close_actually_open'.format(thr))
...
def main(model, devices, args_input, caches):
    ...
    parser.add_argument('--lm_name', default='s-pred-eyeoc-sep-sig,s-pred')
