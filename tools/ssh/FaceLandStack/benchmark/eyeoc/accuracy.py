...
class TestUnit(TestUnitBase):
    def __init__(self, caches):
        name = "eyeoc.accuracy Tester ({}-{})".format(time.strftime("%x"), time.strftime("%X"))
    ...
    def compute_metric(self, lm_name, badcase, patch_name, out_root='/tmp/eyeoc.accuracy'):
        ...
        for key in self._kv_dict_all.keys():
            ...
            res_dict0['#close(1)'] = np.sum(gt)
            res_dict0['#open(0)'] = len(gt) - np.sum(gt)
...
def main(model, devices, args_input, caches):
    ...
    parser.add_argument('--lm_name', default='s-pred-eyeoc,pred')
